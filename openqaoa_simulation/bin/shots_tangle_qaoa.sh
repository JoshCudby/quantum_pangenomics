...
usage()
{
    echo "usage: shots_tangle_qaoa.sh [[-f file] | [-h]]"
}
...
bsub -J "openqaoa" -R '"select[mem>'$memory'] rusage[mem='$memory']"' -gpu - -M "$memory"\
 -o "$outdir/shots.$filename.%J" -e "$outdir/error.shots.$filename.%J" -G "qpg" -q "qpg" \
 "python3 $WORKING_DIR/shots_tangle_qaoa.py $filename"

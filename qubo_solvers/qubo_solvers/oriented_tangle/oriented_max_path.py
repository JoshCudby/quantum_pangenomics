...
filepath, filename, out_dir, graph, time_limit, Q, offset, T_max, V, solver = setup(*sys.argv)
...
if solver == Solver.DWAVE:
    ...
    sample, energy, path = mqlib_sample_qubo(out_dir, filename, offset, graph, T_max, V, time_limit)
...
compile_path = out_dir + f'/{solver.value}.{filename}.compiled.txt'

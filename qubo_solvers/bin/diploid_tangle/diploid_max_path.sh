...
usage()
{
    echo "usage: oriented_max_path [[[-f file] [-j jobs] [-t times] [-m memory] [-s solver]] | [-h]]"
}
...
do
    ...
    bsub -J  "dip_maxpath[1-$jobs]" -R '"select[mem>'$memory'] rusage[mem='$memory']"' -q qpg -gpu - \
     ...
     -G "qpg" "python3 $WORKING_DIR/qubo_solvers/diploid_tangle/diploid_max_path.py $solver $filepath $time_limit"
done
exit 0
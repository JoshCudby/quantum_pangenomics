...
do
    ...
    bsub -J  "mqlibJobs[1-$jobs]" -R '"select[mem>'$memory'] rusage[mem='$memory']"'\
     -M "$memory" -o "$outdir/mqlib.full.$filename.%J.%I" -e "$outdir/error.mqlib.full.$filename.%J"\
      -G "qpg" "python3 qubo_solvers/tangle/max_path_mqlib.py $filepath $normalisation $time_limit"
done

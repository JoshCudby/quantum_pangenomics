"""CVaR-QAOA on IBM hardware with readout bias correction (biased initial state).

Variant of ``optimize_hardware.py`` that prepares a biased initial state via
small-angle Rx rotations (rather than a uniform superposition) before applying
the QAOA ansatz.  The rotation angle ``theta`` is computed from the number of
nodes ``N`` to concentrate initial amplitude on low-Hamming-weight states.
Twirling and dynamical decoupling settings mirror the main hardware script.

CLI usage::

    python optimize_hardware_bias.py -f <filename> -N <nodes> [-p <reps>]
                                     [-a <alpha>] [-M <method>] [-n <shots>]
                                     [--init {ramp,random,fixed}]

Args:
    -f / --filename (str): Base name of the QUBO data ``.pkl`` file.
    -N / --nodes (int): Number of graph nodes, used to set the initial-state
        bias angle.
...
Output:
    Saves a pickle file with the optimisation result, history, and circuit
    graph artefacts to the hardware output directory.
"""
...
def print_circuit_info(qc, circuit_name):
    ...
    Counts CZ, RZZ, CX, and ECR gates (the last being native to ibm_aachen).
...
def objective(x: np.ndarray):
    ...
    Submits the parameterised circuit (with biased initial state) to the IBM
    Sampler, evaluates returned bitstrings against the cost operator with
    Ising offset, and returns the CVaR.  Updates module-level best tracking
    and appends timing data to ``history``.

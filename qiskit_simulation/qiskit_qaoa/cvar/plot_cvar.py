...
with open(f'/lustre/scratch127/qpg/jc59/out/qiskit/cvar/{filename}_cvar.p{p}.hardware{hardware}.pkl', 'rb') as f:
    data = pickle.load(f)
...
axs[1].plot(xsamples, np.cumsum([hist[0] for hist in history]), label="QSim Time")
axs[1].plot(xsamples, np.cumsum([hist[4] for hist in history]), label="Classical Time")
...
axs[1].set_ylabel('Runtime with queueing (s)')
axs[2].set_xlabel('Total number of samples')
axs[2].set_ylabel('Runtime without queueing (s)')
axs[2].set_yscale('log')
fig.tight_layout()

fig.savefig(f'/nfs/users/nfs_j/jc59/quantumwork/pangenome/qiskit_simulation/out/{filename}.p{p}.hardware{hardware}.convergence.png')
...
rand_vals = [qp.objective.evaluate(sample) for sample in random_samples[2]]
...
fig.savefig(f'/nfs/users/nfs_j/jc59/quantumwork/pangenome/qiskit_simulation/out/{filename}.p{p}.hardware{hardware}.histogram.png')

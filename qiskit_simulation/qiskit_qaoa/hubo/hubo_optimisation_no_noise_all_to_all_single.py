...
p = 3
param = [ 6.058e-01,  3.503e-01,  3.691e-01,  1.844e-01,  9.353e-01, 9.155e-01]
result = minimize(
    objective, 
    x0=param, 
    method=args.method, 
    bounds=tuple((0,1) for _ in range(2 * p)), 
    options={"maxiter": 150, "rhobeg": 0.01},  # , "ftol": 1e-7
...
obj_to_dump = dict(
    result=result, history=history, t_qaoa_circ=t_qaoa_circ, hamiltonian=full_hamiltonian, 
)

dump_file = f'/lustre/scratch127/qpg/jc59/out/qiskit/hubo_no_shot_noise_optimum/single.{filename}.p{p}.pkl'

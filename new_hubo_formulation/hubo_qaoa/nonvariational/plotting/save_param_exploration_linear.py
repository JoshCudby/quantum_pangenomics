...
for inst in range(n_instances):
    for p_idx, p in enumerate(p_values):
        ...
        print(instance_names[inst], p, delta_beta[ix], delta_gamma[iy])
        ax.plot(
            x_max, y_max,
            marker='x',
            color='red',
...
fig.text(x_center, y_below_heatmaps, r'$\Delta_\beta$', ha='center', va='top', fontsize=16)
...
for inst in range(n_instances):
    ax_summary.plot(
        np.round(np.array(p_values), 1),
        np.log10(best_vals[inst]),
        marker="o",
        label=n_values[inst],
    )

ax_summary.set_xlabel(r"$p$", fontsize=14)
ax_summary.set_ylabel(r"Best $\log_{10}(p_{\mathrm{opt}})$", fontsize=14)
ax_summary.set_ylim(-4, 0)
ax_summary.set_xticks(np.round(np.array(p_values), 1))

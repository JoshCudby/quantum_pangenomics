...
zero_indexes = np.nonzero(evals < 1e-5)
...
def LR_QAOA(p: int, delta_b: float, delta_g: float, circ: Optional[QuantumCircuit]):
    ...
    logger.info(f'delta_b:{np.round(delta_b, 2)}, delta_g:{np.round(delta_g, 2)}, p:{p}, probability:{np.round(probability, 4)}')
    return probability, circuit
...
to_save_name = f"/lustre/scratch127/qpg/jc59/new_hubo_formulation/nonvariational/param_exploration/LR_equal.performance.{filename}.db{delta_b_fixed}.dg{delta_g_fixed}.rescaling{np.round(rescaling[-1],2)}.p{ps[-1]}.pkl"

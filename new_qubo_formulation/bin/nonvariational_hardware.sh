...
usage()
{
    echo "usage: nonvariational_hardware.sh [[-f file -m memory -n shots -N nodes] | [-h]]"
}
...
outdir="$SCRATCH/new_qubo_formulation/oriented/nonvariational"
...
bsub -J "nonvar_hardware_qubo_$filename" -R '"select[mem>'$memory'] rusage[mem='$memory']"' -M "$memory"\
 ...
 "python3 $WORKING_DIR/nonvariational_hardware.py -f $filename -N $nodes -n $shots"

...
usage()
{
    echo "usage: plot_oriented_prog_qaoa.sh [[-f file -m memory -M memory -p reps -n shots -i maxiter --init] | [-h]]"
}
...
bsub -J "plot_prog_qaoa" -R '"select[mem>'$memory'] rusage[mem='$memory']"' -M "$memory"\
 ...
 "python3 $WORKING_DIR/plot_oriented_prog_qaoa.py -f $filename -p $reps -n $shots --init $init -i $maxiter -M $method"

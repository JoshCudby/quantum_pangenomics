...
usage()
{
    echo "usage: optimize_oriented.sh [[-f file -m memory -M method -b blocking -p reps -n shots -g num_gpu -i max_iter --init] | [-h]]"
}
...
bsub -J "optimize_$filename" -R '"select[mem>'$memory'] rusage[mem='$memory']"' -gpu "num=$num_gpu:aff=no:j_exclusive=yes" -M "$memory"\
 ...
 "python3 $WORKING_DIR/optimize_oriented_prog_qaoa.py -f $filename -p $reps -b $blocking -m $memory -n $shots -i $maxiter --init $init -M $method"
